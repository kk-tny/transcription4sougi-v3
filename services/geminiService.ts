...
4. 問い合わせの種類: 以下の定義に基づき、最も適切なものを1つ選んでください。
   ...
   - 無音・会話なし（応答なし）: 音声に人の会話が全く含まれない場合（無音、呼び出し音・ベル音のみ、ワンギリ・すぐに切れた通話、ノイズのみ、応答なしなど）
...
【最重要ルール：無音・会話なし（応答なし）時のハルシネーション絶対禁止】
会話・人の声が全く聞き取れない音声（無音、ワンギリ、呼び出し音のみ、雑音・ノイズのみ等）の場合は、絶対に架空の会話や人物名・内容を創作（ハルシネーション）しないでください。人の会話が存在しない場合は、必ず以下の通り固定で出力してください：
- ご相談者様名: "不明"
- ご対象者様名: "不明"
- 応対者名: ["不明（名乗らず）"]
- 問い合わせの種類: "無音・会話なし（応答なし）"
- 具体的な内容: ["・無音または会話なしの通話"]
- 文字起こし: []（空の配列）`,
      ...
      responseSchema: {
        ...
        properties: {
          ...
          inquiryType: { 
            type: Type.STRING,
            enum: ['訃報', '訃報（依頼済みの方から再連絡）', '事前相談', '自身の事前相談', '参列問い合わせ', '供花、供物の注文依頼', '間違い電話（葬儀相談）', '間違い電話（葬儀相談以外）', '間違い電話（葬儀社以外宛）', '無音・会話なし（応答なし）', 'その他']
          },
  ...
  const parsed: TranscriptionResult = JSON.parse(text);

  // 万が一 AIが「無音・会話なし（応答なし）」と判定した、あるいは文字起こしが空の場合の安全補正ガード
  if (parsed.inquiryType === '無音・会話なし（応答なし）' || !parsed.transcription || parsed.transcription.length === 0) {
    parsed.callerName = "不明";
    parsed.subjectName = "不明";
    parsed.responderNames = ["不明（名乗らず）"];
    parsed.inquiryType = "無音・会話なし（応答なし）";
    parsed.details = ["・無音または会話なしの通話"];
